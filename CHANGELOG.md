...
- UniFi Network Controller integration. A new backend service and router fetch access points and switches from the UniFi Network API and upsert them into the Device Inventory as `unifi` devices, with STP priority and uplink MAC resolution; known and active clients come in alongside the infrastructure. Host, site and credentials are configured in a UniFi panel in Settings, with an optional scheduled sync. An existing device is matched on its IEEE address first and only falls back to a MAC when the candidate carries none, so a client is merged into the row another source already owns rather than duplicated. Thanks @slmingol! (#433)
- The documentation space can be shared read-only with people who have no account, the way Live View already shares a canvas. One new `DOCS_VIEW_KEY` in `.env` turns it on — unset, every public route answers 403 before the key is compared — and the page at `/docs?key=` renders the tree, the filter and the viewer with the app's own components. The public router declares no write route at all, the payload is built field by field so nothing the key does not grant leaks, and the store behind the page is separate from the authenticated one rather than a flag on it. A **Share link** button appears in the Documentation footer when the server says the key is set. Anyone holding the URL reads every document. (#501)
- The OpenAPI schema, Swagger UI and ReDoc are reachable in a packaged install. `/docs`, `/redoc` and `/openapi.json` sit outside `/api/`, so until now they fell through to the SPA fallback and returned the React app; nginx proxies all three in the Docker, subpath and bare-metal configs. A new `docs/api.md` covers the bearer-token login flow, an endpoint table, worked examples for creating, updating and linking nodes, and the security caveats — the three paths are not authenticated, and expose the API's shape, not homelab data. (#500)
- Zigbee LQI is carried on the link instead of the device, and the mesh links the parent tree drops can be imported on demand. A new `edges.lqi` column and a read-only row in the edge modal show the measurement where it belongs; `device_inventory_links.lqi` is finally populated. The opt-in `include_mesh_links` import adds a `zigbee_mesh` edge type with its own colour in all six themes, off by default and excluded from the scheduled sync — a 37-device network goes from roughly 36 edges to 115. The Scan History icon spins while a scan runs. (#498)
- The MCP server reaches the documentation space: nine tools covering full-text search, listing, reading, revisions and backlinks, plus create, update and restore. There is no delete tool — destroying documentation stays a human action. Every AI edit is recorded in the document's history under a `mcp` reason the client cannot forge, and the history rail labels it "Saved by an AI client". (#497)
- Every import lives behind one **Import from…** picker in the left rail. One tile per source with its logo, what it brings in and a rough duration, so a fifth integration costs a tile instead of another sidebar link. The source modals themselves are untouched. (#505)

### Fixes

- A patch panel port on the rack canvas accepted a single cable, so a panel could never show both halves of the run it exists to carry. It now takes two — every other port kind still takes exactly one, since a switch jack really is one cable. Two bugs sat behind the limit: a cable's hit area painted above the plates and swallowed the release, so the second patch could not be drawn at all, and a refused patch vanished on release with no explanation. A patched socket now takes the colour of the first cable on it rather than the last. (#507)
- Two services on the same port — a node behind a reverse proxy serving several sites — collapsed into one, and the canvas then saved back the shortened list, deleting the service for good. A service's identity is now the port, protocol, host and path, with older view keys migrated forward on read and a port-only fallback kept for scanner input, which reports a port and never the site served on it. This was a regression from the #468 / #469 identity fixes meeting the per-service host override that shipped a month earlier. (#506)
- An uninitialized MCP backend client leaked an `AttributeError`; it now answers a clear `BackendError` with status 503. Thanks @kreativmonkey! (#483)
- Setting a tag on a document could silently destroy unsaved work. Tags live in the body's frontmatter, so clicking a chip writes the whole body and moves `updated_at`, which the locally mirrored draft is keyed on — the next open dropped it as stale. The draft is now re-based onto the new `updated_at` with the same tag rewrite applied, and is still never sent to the server. Thanks @slmingol! (#459)
- A manual UniFi import left no trace in Scan History. UniFi is the only importer that runs inline rather than as a background job, so no "running" row was ever written; both the manual import and sync-now now record a terminal `scan_runs` row, and a failed import is recorded as an `error` run rather than staying silent. (#505)
- The API advertised version `1.9.0` — a hard-coded literal frozen in April 2026, ten releases ago. FastAPI now reads `APP_VERSION` from the `VERSION` file, and a new test pins every link in that chain. (#500)
- Bumped vitest to 4.1.11 for the mocker path traversal advisory. (#500)
